...
system(['rm -rf ',Path_GeoLAIadj]);
system(['mkdir -p ',Path_GeoLAIadj]);
...
% 删除含有 NaN 的行
NanMask = any(isnan(X), 2) | isnan(GeoMulLAI);
Xclean = X(~NanMask, :);  % 删除含有 NaN 的行
LAIclean = GeoMulLAI(~NanMask);  % 删除响应变量的 NaN 行
...
if ~isempty(Xclean)
    ...
    % 使用岭回归（可以避免共线性问题）
    lambda = 0.1;  % 正则化参数
    b = ridge(LAIclean, Xclean(:, 2:end), lambda, 0);  % 忽略偏置项列

    % 计算预测值和残差
    ypred = Xclean * b;  % 预测 LAI
    residuals = LAIclean - ypred;  % 计算残差

    % 初始化为 NaN
        AdjustValue = nan(size(GeoMulLAI));
...
GeoLAIadj = reshape(AdjustValue, [18000, 43200]);

% 将调整后的 LAI 保存
geotiffwrite(fullfile(Path_GeoLAIadj,'LAIadj_B2001E2020_CMG1KM.tif'), GeoLAIadj, R, ...
