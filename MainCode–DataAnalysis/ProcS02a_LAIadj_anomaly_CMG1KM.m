...
Path_GeoLAIadj = '../output/S01a_GlobeLAIadj_CMG1KM/';
Path_LAIanomaly = '../output/S02a_LAIadj_anomaly_CMG1KM/';
...
% 读取 LAI 数据
File_LAI = fullfile(Path_GeoLAIadj,'LAIadj_B2001E2020_CMG1KM.tif');
GeoLAIadj = double(readgeoraster(File_LAI));
...
for Index = 1 : WinSize : size(GeoAsType,1)
    for j =  1 : WinSize : size(GeoAsType,2)   
        ...
        WinLAI = GeoLAIadj(StartRow, StartCol);
        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioE + RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                LAIcontAvg = nanmean(LAIcontrol); 
                LAIcontStd = nanstd(LAIcontrol);
                ...
                TargetAs = WinLAI(WinAs == 2 | WinAs == 3| WinAs == 4); % 高+中风险区
                ...
                if  nnz(~isnan(TargetAs)) < 2                
                   continue;
                    ...
                    WinLAIanomaly(WinAs == 4 | WinAs == 2 | WinAs == 3) = ...
                        (WinLAI(WinAs == 4 | WinAs == 2 | WinAs == 3) - LAIcontAvg) / LAIcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_LAIanomaly, 'LAIadj_anomaly_CMG1KM.tif'), GeoLAIanomaly, R, ...
             'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_LAIanomaly, 'LAIcontAvg_CMG1KM.tif'), GeoLAIcontAvg, R, ...
             'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_LAIanomaly, 'LAIcontStd_CMG1KM.tif'), GeoLAIcontStd, R, ...
             'TiffTags', struct('Compression', Tiff.Compression.LZW));
