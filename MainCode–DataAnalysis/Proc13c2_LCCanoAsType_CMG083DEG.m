...
Path_LCCanomaly = '../output/13c1_LCCanoAsType_CMG1KM/';
Path_Anomaly_CMG083DEG = '../output/13c2_LCCanoAsType_CMG083DEG/';
...
% 读取 LCC anomaly 数据
LCCanomaly = readgeoraster([Path_LCCanomaly, 'LCCanoAsType_CMG1KM.tif']);
...
for row = 1:numRows
    for col = 1:numCols
        ...
        LRAsMean(row, col) = mean(WinLCC(WinAsT == 1),'omitnan');
        MRAsMean(row, col) = mean(WinLCC(WinAsT == 2),'omitnan');
        HRAsMean(row, col) = mean(WinLCC(WinAsT == 3),'omitnan');
        ERAsMean(row, col) = mean(WinLCC(WinAsT == 4),'omitnan'); 

        % 计算ERAs,HRAs, MRAs, LRAs各自的标准差（过滤 NaN）
        LRAsSTD(row, col) = std(WinLCC(WinAsT == 1),'omitnan');
        MRAsSTD(row, col) = std(WinLCC(WinAsT == 2),'omitnan');
        HRAsSTD(row, col) = std(WinLCC(WinAsT == 3),'omitnan');
        ERAsSTD(row, col) = std(WinLCC(WinAsT == 4),'omitnan'); 
...
% 保存 HRAs, MRAs, LRAs 的 LCC anomaly 均值
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_LRAsMean_CMG083DEG.tif'), LRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_MRAsMean_CMG083DEG.tif'), MRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_HRAsMean_CMG083DEG.tif'), HRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_ERAsMean_CMG083DEG.tif'), ERAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));

% 保存 HRAs, MRAs, LRAs 的 LCC anomaly 标准差
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_LRAsSTD_CMG083DEG.tif'), LRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_MRAsSTD_CMG083DEG.tif'), MRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_HRAsSTD_CMG083DEG.tif'), HRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLCC_ERAsSTD_CMG083DEG.tif'), ERAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
