...
Path_SIFanomaly = '../output/12a_SIFanomaly_CMG1KM/';
...
GeoMulSIF(LandCover==15 | LandCover==16) = nan;
       ...
       % % 如果窗口内没有效像元直接跳过
        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioE + RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                TargetAs = WinSIF(WinAs == 2 | WinAs == 3 | WinAs == 4); % 极高+高+中风险区
                ...
                if  nnz(~isnan(TargetAs)) < 2                
                   continue;
                    ...
                    WinSIFanomaly(WinAs == 4 |WinAs == 2 | WinAs == 3) = ...
                        (WinSIF(WinAs == 4 |WinAs == 2 | WinAs == 3) - SIFcontAvg) / SIFcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_SIFanomaly, 'SIFanomaly_CMG1KM.tif'), GeoSIFanomaly, R, ...
