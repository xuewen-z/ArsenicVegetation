    ...
    for j =  1 : WinSize : size(GeoAsType,2)   
        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioE + RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                TargetAs = WinLAI(WinAs == 2 | WinAs == 3 | WinAs == 4); % 极高+高+中风险区
                ...
                if  nnz(~isnan(TargetAs)) < 2                
                   continue;
                    ...
                    WinLAIanomaly(WinAs == 4 | WinAs == 3 | WinAs == 2) = ...
                        (WinLAI(WinAs == 4 | WinAs == 3 | WinAs == 2) - LAIcontAvg) / LAIcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_LAIanomaly, 'LAIanomaly_CMG1KM.tif'), GeoLAIanomaly, R, ...
