...
system(['rm -rf ',Path_GeoLCCadj]);
system(['mkdir -p ',Path_GeoLCCadj]);
...
% 删除含有 NaN 的行
NanMask = any(isnan(X), 2) | isnan(GeoMulLCC);
Xclean = X(~NanMask, :);  % 删除含有 NaN 的行
LCCclean = GeoMulLCC(~NanMask);  % 删除响应变量的 NaN 行
...
if ~isempty(Xclean)
    ...
    % 使用岭回归（可以避免共线性问题）
    lambda = 0.1;  % 正则化参数
    b = ridge(LCCclean, Xclean(:, 2:end), lambda, 0);  % 忽略偏置项列

    % 计算预测值和残差
    ypred = Xclean * b;  % 预测 LCC
    residuals = LCCclean - ypred;  % 计算残差

    % 初始化为 NaN
        AdjustValue = nan(size(GeoMulLCC));
...
GeoLCCadj = reshape(AdjustValue, [18000, 43200]);

% 将调整后的 LCC 保存
geotiffwrite(fullfile(Path_GeoLCCadj,'LCCadj_B2001E2020_CMG1KM.tif'), GeoLCCadj, R, ...
