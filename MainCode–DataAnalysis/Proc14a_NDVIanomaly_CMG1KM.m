...
Path_NDVIanomaly = '../output/14a_NDVIanomaly_CMG1KM/';
...
GeoMulNDVI(LandCover==15 | LandCover ==16) =nan;
    ...
    for j =  1 : WinSize : size(GeoAsType,2)   
        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioE + RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                TargetAs = WinNDVI(WinAs == 2 | WinAs == 3| WinAs == 4); % 极高+高+中风险区
                ...
                if  nnz(~isnan(TargetAs)) < 2                
                   continue;
                    ...
                    WinNDVIanomaly(WinAs == 4 |WinAs == 2 | WinAs == 3) = ...
                        (WinNDVI(WinAs == 4 |WinAs == 2 | WinAs == 3) - NDVIcontAvg) / NDVIcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_NDVIanomaly, 'NDVIanomaly_CMG1KM.tif'), GeoNDVIanomaly, R, ...
