...
Path_Anomaly_CMG1KM = '../output/S02a_LAIadj_anomaly_CMG1KM/';
Path_Anomaly_CMG083DEG = '../output/S02b_LAIadj_anomaly_CMG083DEG/';
...
% 读取 LAI anomaly 数据
LAIanomaly = readgeoraster([Path_Anomaly_CMG1KM, 'LAIadj_anomaly_CMG1KM.tif']);
        ...
        for j = 1 : WinSize : size(LAIanomaly,2)   
            NumCol = NumCol + 1; 
                       ...
                       if   numel(LAIkm(~isnan(LAIkm))) >= 2
                            ...
                            % Wilcoxon 符号秩检验（检验 HRAs 和 MRAs 的 LAI anomaly 是否显著 ≠ 0）
...
% 保存 LAI Anomaly 结果
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAIadj_CMG083DEG.tif'), AnomalyLAI, R, ...
    'TiffTags', struct('Compression', Tiff.Compression.LZW));

% 保存 Wilcoxon 符号秩检验结果
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'SignifiLAIadj_CMG083DEG.tif'), SignifiLAI, R, ...
    'TiffTags', struct('Compression', Tiff.Compression.LZW));
