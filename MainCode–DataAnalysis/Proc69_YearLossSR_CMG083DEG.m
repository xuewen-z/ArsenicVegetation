...
system(['rm -rf '  ,Path_YearLossSR]);
system(['mkdir -p ',Path_YearLossSR]);
...
for Year = 2002 : 2019
    ...
    SR = readgeoraster([Path_MulYSR,'EcoServ_SR.Y',YearName,'_CMG1KM.tif']);
...
FileName =[Path_YearLossSR,'WinSRLoss.Y',YearName,'_CMG083DEG.tif'];
geotiffwrite(FileName,LossSR,R,'TiffTags',struct('Compression',Tiff.Compression.LZW));

FileName =[Path_YearLossSR,'WinSRTotal.Y',YearName,'_CMG083DEG.tif'];
geotiffwrite(FileName,TotalSR,R,'TiffTags',struct('Compression',Tiff.Compression.LZW));


LossSR(LossSR>0) = nan;
LossSRsum = sum(LossSR(:),'omitnan'); 
CO2LossSR(:,Year-2001) = LossSRsum;
...
save([Path_YearLossSR, 'CO2LossSR.mat'], 'CO2LossSR');  
