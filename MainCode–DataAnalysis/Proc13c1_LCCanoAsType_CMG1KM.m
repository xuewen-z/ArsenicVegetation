        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioE + RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                LCCcontStd(LCCcontStd < 1e-3) = 0.01;
                    ...
                    WinLCCanomaly(WinAs == 4 | WinAs == 3 | WinAs == 2| WinAs == 1) = ...
                        (WinLCC(WinAs == 4 | WinAs == 3 | WinAs == 2| WinAs == 1) - LCCcontAvg) / LCCcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_LCCanomaly, 'LCCanoAsType_CMG1KM.tif'), GeoLCCanomaly, R, ...
