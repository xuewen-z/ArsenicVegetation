...
Path_Anomaly_CMG083DEG = '../output/11c2_LAIanoAsType_CMG083DEG/';
...
% 读取 LAI anomaly 数据
LAIanomaly = readgeoraster([Path_LAIanomaly, 'LAIanoAsType_CMG1KM.tif']);
...
% 计算网格尺寸
numRows = ceil(size(LAIanomaly, 1) / WinSize);
numCols = ceil(size(LAIanomaly, 2) / WinSize);
...
for row = 1:numRows
    for col = 1:numCols
        ...
        EndRow = min(StartRow + WinSize - 1, size(LAIanomaly, 1));
        EndCol = min(StartCol + WinSize - 1, size(LAIanomaly, 2));

     
        % 提取窗口数据
        WinLAI = LAIanomaly(StartRow:EndRow, StartCol:EndCol);
        WinAsT = GeoAsType(StartRow:EndRow, StartCol:EndCol);

     
        % 计算ERAs, HRAs, MRAs, LRAs各自的均值（过滤 NaN）
        LRAsMean(row, col) = mean(WinLAI(WinAsT == 1),'omitnan');
        MRAsMean(row, col) = mean(WinLAI(WinAsT == 2),'omitnan');
        HRAsMean(row, col) = mean(WinLAI(WinAsT == 3),'omitnan');
        ERAsMean(row, col) = mean(WinLAI(WinAsT == 4),'omitnan'); 

        % 计算ERAs,HRAs, MRAs, LRAs各自的标准差（过滤 NaN）
        LRAsSTD(row, col) = std(WinLAI(WinAsT == 1),'omitnan');
        MRAsSTD(row, col) = std(WinLAI(WinAsT == 2),'omitnan');
        HRAsSTD(row, col) = std(WinLAI(WinAsT == 3),'omitnan');
        ERAsSTD(row, col) = std(WinLAI(WinAsT == 4),'omitnan'); 
...
% 保存 HRAs, MRAs, LRAs 的 LAI anomaly 均值
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_LRAsMean_CMG083DEG.tif'), LRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_MRAsMean_CMG083DEG.tif'), MRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_HRAsMean_CMG083DEG.tif'), HRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_ERAsMean_CMG083DEG.tif'), ERAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));

% 保存 HRAs, MRAs, LRAs 的 LAI anomaly 标准差
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_LRAsSTD_CMG083DEG.tif'), LRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_MRAsSTD_CMG083DEG.tif'), MRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_HRAsSTD_CMG083DEG.tif'), HRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyLAI_ERAsSTD_CMG083DEG.tif'), ERAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
