...
Path_Anomaly_CMG083DEG = '../output/14c2_NDVIanoAsType_CMG083DEG/';
...
% 读取 NDVI anomaly 数据
NDVIanomaly = readgeoraster([Path_NDVIanomaly, 'NDVIanoAsType_CMG1KM.tif']);
...
for row = 1:numRows
    for col = 1:numCols
        ...
        LRAsMean(row, col) = mean(WinNDVI(WinAsT == 1),'omitnan');
        MRAsMean(row, col) = mean(WinNDVI(WinAsT == 2),'omitnan');
        HRAsMean(row, col) = mean(WinNDVI(WinAsT == 3),'omitnan');
        ERAsMean(row, col) = mean(WinNDVI(WinAsT == 4),'omitnan'); 

        % 计算ERAs,HRAs, MRAs, LRAs各自的标准差（过滤 NaN）
        LRAsSTD(row, col) = std(WinNDVI(WinAsT == 1),'omitnan');
        MRAsSTD(row, col) = std(WinNDVI(WinAsT == 2),'omitnan');
        HRAsSTD(row, col) = std(WinNDVI(WinAsT == 3),'omitnan');
        ERAsSTD(row, col) = std(WinNDVI(WinAsT == 4),'omitnan'); 
...
% 保存 HRAs, MRAs, LRAs 的 NDVI anomaly 均值
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_LRAsMean_CMG083DEG.tif'), LRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_MRAsMean_CMG083DEG.tif'), MRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_HRAsMean_CMG083DEG.tif'), HRAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_ERAsMean_CMG083DEG.tif'), ERAsMean, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));

% 保存 HRAs, MRAs, LRAs 的 NDVI anomaly 标准差
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_LRAsSTD_CMG083DEG.tif'), LRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_MRAsSTD_CMG083DEG.tif'), MRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_HRAsSTD_CMG083DEG.tif'), HRAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
geotiffwrite(fullfile(Path_Anomaly_CMG083DEG, 'AnomalyNDVI_ERAsSTD_CMG083DEG.tif'), ERAsSTD, R, 'TiffTags', struct('Compression', Tiff.Compression.LZW));
