...
system(['rm -rf ',Path_GeoSIFadj]);
system(['mkdir -p ',Path_GeoSIFadj]);
...
% 删除含有 NaN 的行
NanMask = any(isnan(X), 2) | isnan(GeoMulSIF);
Xclean = X(~NanMask, :);  % 删除含有 NaN 的行
SIFclean = GeoMulSIF(~NanMask);  % 删除响应变量的 NaN 行
...
if ~isempty(Xclean)
    ...
    % 使用岭回归（可以避免共线性问题）
    lambda = 0.1;  % 正则化参数
    b = ridge(SIFclean, Xclean(:, 2:end), lambda, 0);  % 忽略偏置项列

    % 计算预测值和残差
    ypred = Xclean * b;  % 预测 SIF
    residuals = SIFclean - ypred;  % 计算残差

    % 初始化为 NaN
        AdjustValue = nan(size(GeoMulSIF));
...
GeoSIFadj = reshape(AdjustValue, [18000, 43200]);

% 将调整后的 SIF 保存
geotiffwrite(fullfile(Path_GeoSIFadj,'SIFadj_B2001E2020_CMG1KM.tif'), GeoSIFadj, R, ...
