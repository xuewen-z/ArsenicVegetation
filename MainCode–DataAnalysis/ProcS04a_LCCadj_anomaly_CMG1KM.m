...
Path_GeoLCCadj = '../output/S01c_GlobeLCCadj_CMG1KM/';
Path_LCCanomaly = '../output/S04a_LCCadj_anomaly_CMG1KM/';
...
% 读取 LCC 数据
File_LCC = fullfile(Path_GeoLCCadj,'LCCadj_B2001E2020_CMG1KM.tif');
GeoLCCadj = double(readgeoraster(File_LCC));
...
for Index = 1 : WinSize : size(GeoAsType,1)
    for j =  1 : WinSize : size(GeoAsType,2)   
        ...
        WinLCC = GeoLCCadj(StartRow, StartCol);
        ...
        % 筛选条件：HRAs + MRAs ≥ 5% 且 LRAs ≥ 5%
            if (RatioH + RatioM) >= AsThreshold && RatioL >= AsThreshold
                ...
                LCCcontAvg = nanmean(LCCcontrol); 
                LCCcontStd = nanstd(LCCcontrol);
                ...
                TargetAs = WinLCC(WinAs == 2 | WinAs == 3); % 高+中风险区
                ...
                if  nnz(~isnan(TargetAs)) < 2                
                   continue;
                    ...
                    WinLCCcontAvg(WinAs == 3 | WinAs == 2 |WinAs == 1) = LCCcontAvg;
                    WinLCCcontStd(WinAs == 3 | WinAs == 2 |WinAs == 1) = LCCcontStd;
    
            
                     % 计算 LCC anomaly
                    WinLCCanomaly = nan(size(WinLCC)); 
                    WinLCCanomaly(WinAs == 2 | WinAs == 3) = ...
                        (WinLCC(WinAs == 2 | WinAs == 3) - LCCcontAvg) / LCCcontStd;
...
% 保存 GeoTIFF 结果
geotiffwrite(fullfile(Path_LCCanomaly, 'LCCadj_anomaly_CMG1KM.tif'), GeoLCCanomaly, R, ...
